...
\def\MyAlignedGC#1{\rlap{#1}\hspace*{9em}\ignorespaces}
\begin{tabular}{|crlc|}
\hline%
...
hardwired output of $C_\GC'$ at ${j=i_\bot^\ast}$;
& \MyMiniSectionSkip \\ &
\textbf{Input. }&
...
&\textbf{if }{\color{red}\boxed{\smash{j=i_\bot^\ast}\vphantom{.^.}}}\textup{:}\\
&\qquad\textbf{output }
(\widehat{C}_{\ct,i_\bot^\ast},\{\LOT.\ct_{i_\bot^\ast,m_0}\}_{m_0\in[M_0]})
\textup{ as hardwired}\\
&\textbf{else}\textup{:}\\
&\qquad\begin{aligned}
r^\GC_j&{}\gets\PPRF.\Eval(\mathring{k}^\GC_{i_\bot^\ast},j)\\
r^\LOT_{j,m_0}&{}\gets\PPRF.\Eval(\mathring{k}^\LOT_{m_0,i_\bot^\ast},j)\quad\textup{for }m_0\in[M_0]\\
r^\PKE_j&{}\gets\PPRF.\Eval(\mathring{k}^\PKE_{i_\bot^\ast},j)\\
(\widehat{C}_{\ct,j},{}&\{L_{j,m_0,b}\}_{m_0\in[M_0],b\in\bit})\\
&{}\gets\begin{cases}
\GC.\Garble(\widehat{C}_\ct,(\mu_\bot,r^\PKE_j);r^\GC_j),&
\textup{if }{\color{red}\boxed{\smash{j<i_\bot^\ast}\vphantom{.^.}}}\,;\\
\GC.\Garble(\widehat{C}_\ct,(\mu_{\phantom{\bot}},r^\PKE_j);r^\GC_j),&
\textup{if }{\color{red}\boxed{\smash{j>i_\bot^\ast}\vphantom{.^.}}}\,;
\end{cases}\\
\mathllap{\LOT.\ct_{j,m_0}}&{}\gets
