...
\luoji{
Things to do:
\par(0) Check space factor.
% Regular expression is (case-sensitive) [A-Z](\}|\)|\]|')*(\.|\?|!|:|;|,)
\par(1) Check first-page footnote.
\par(2) Check script positioning.
% Consider using \smash and placeholder script to force alignment.
\par(3) Check special heading of ``You Can (Not) Optimize''.
\par(4) No horizontal/vertical overflow/underflow.
% Reword. Hint break points.
\par(5) Place figures.
\par(6) Ensure each footnote text is on the same page as its mark.
}
...
This is the author-formatted version of
an article with the same title,\\
DOI \href{https://doi.org/10.62056/a39qxrxqi}{\texttt{10.62056/a39qxrxqi}},
in IACR CiC volume 1 issue 2
published by the IACR\@.\\
